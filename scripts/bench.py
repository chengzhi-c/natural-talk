...
门禁:
  ...
  - detector 10KB <1.0ms 且比旧 2.1ms 快 2x
  - cases F1 >0.93
"""
import json
import time
...
def check_budgets():
    budgets = {
        ...
        "dist/prompts/prompt.l1.txt": (210, 110),
        "dist/prompts/prompt.l2.txt": (420, 190),
        "dist/SKILL.md": (1350, 620),
        "dist/prompts/prompt.l3.txt": (700, 320),
    }
    ok = True
...
def check_speed():
    try:
        from engine.detector import run_checks, article_check
    except ImportError:
    ...
    status = "PASS" if dt < 6.0 else "FAIL"
    print(f"[{status}] detector 10KB {dt:.2f} ms/次 (基线2.1ms, 门禁6ms, 加速{2.1/dt:.1f}x)")
    return status == "PASS"
...
def check_f1():
    ...
    tp=fp=fn=tn=0
    for c in data["cases"]:
        # 兼容文章版 title/body 与对话版 user/answer
        if "title" in c:
            from engine.detector import article_check
            v,_ = article_check(c["title"], c["body"])
        else:
