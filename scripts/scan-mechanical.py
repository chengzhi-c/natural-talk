...
覆盖规则分两级：
  ...
  B1  翻案腔：句中含"而是/其实/恰恰"，或省"而"变体"不是A，(只/纯)是B"
      ...
      分隔同判），或"与其说A，不如说B""谈不上A，更多是B""没有A，只有/只是B"
...
# 与破折号分隔形态（"不是A——是B"，单双破折号与句号分隔同判："不是A。是B"）。
# 家族扩展：没有A，只有/只是B（虚实对举；事实清点交模型复核）、不在于A而在于B、
...
# B15 舞台剧抽搐候选（含 scan_slop 合并：下颌紧绷/喉结滚动族/倒吸气/
...
# ---------- scan_slop 合并词条（P3–P8 族，gen 模式 REVIEW 候选） ----------
...
# P8 客服套话 → D2 扩展（非常好问题/希望有所帮助/如您所见族）
