...
破折号 ≤2、感叹号 ≤3、路标词 ≤2 这类阈值没有测量依据（人类破折号约 0.80/千字，300 字折算不足一个，"≤2"宽松到不会生效），而"首先其次 ≤1"被大样本对照直接证伪，正文顺序词两侧无差别。现行规则用可定位的触发标记加反清单，见 `SKILL.md` 文本层。`core/rules.yaml` 已冻结为历史存档。
